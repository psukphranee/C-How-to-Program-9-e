...
vector<float> TemporalFilter::update(vector<float> &reading){
  ...
  //return the median
  vector<float> medianVector = calculateMedianVector();
  return medianVector;
  //printVector(medianVector);
}

void TemporalFilter::addToScanBuffer(vector<float> &reading){
  //cout << "addToScanBuffer(): " << &reading << endl;
